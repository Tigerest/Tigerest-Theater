...
void TestLog::testCensorAuthTokens_data()
{
  ...
  QTest::newRow("X-Emby-Token query")
    << "url?X-Emby-Token=abcdef0123456789abcdef0123456789&other=1"
    << "url?X-Emby-Token=" + masked32 + "&other=1";

  QTest::newRow("X-Emby authorization token")
    << "X-Emby-Authorization: MediaBrowser Token=\"abcdef0123456789abcdef0123456789\""
    << "X-Emby-Authorization: MediaBrowser Token=\"" + masked32 + "\"";

  QTest::newRow("X-Emby mpv header token")
    << "http-header-fields=[\"X-Emby-Token: abcdef0123456789abcdef0123456789\"]"
    << "http-header-fields=[\"X-Emby-Token: " + masked32 + "\"]";

  QTest::newRow("X-MediaBrowser mpv header token")
    << "X-MediaBrowser-Token: abcdef0123456789abcdef0123456789"
    << "X-MediaBrowser-Token: " + masked32;

  QTest::newRow("bearer authorization header")
    << "Authorization: Bearer abcdef0123456789abcdef0123456789"
    << "Authorization: Bearer " + masked32;
