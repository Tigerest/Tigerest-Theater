...
configure_file(
  ...
  ${CMAKE_CURRENT_BINARY_DIR}/TigerestTheater.iss
  @ONLY
)
...
set(INSTALLER_BASE_NAME "TigerestTheater-${VERSION_STRING}-${INSTALLER_ARCH_STR}")
set(INSTALLER_OUTPUT_NAME "${INSTALLER_BASE_NAME}.exe")

# Create the installer using Inno Setup.  Keep this as an always-run target: the
# staged application is refreshed by innosetup_install, so an old package must
# never be considered up-to-date merely because its output file already exists.
add_custom_target(TigerestTheaterInstaller
  DEPENDS innosetup_install ${CMAKE_CURRENT_BINARY_DIR}/TigerestTheater.iss
  COMMAND ${CMAKE_COMMAND} -E rm -f ${CMAKE_CURRENT_BINARY_DIR}/${INSTALLER_OUTPUT_NAME}
  COMMAND ${ISCC}
...
add_custom_target(windows_package DEPENDS TigerestTheaterInstaller)
...
# Like the installer target, always refresh the portable archive after staging.
add_custom_target(TigerestTheaterZip
  ...
  COMMAND ${CMAKE_COMMAND} -E rm -f ${CMAKE_CURRENT_BINARY_DIR}/${ZIP_OUTPUT_NAME}
...
add_custom_target(windows_zip DEPENDS TigerestTheaterZip)

# Combined target for both installer and ZIP
add_custom_target(windows_all DEPENDS TigerestTheaterInstaller TigerestTheaterZip)
