...
{
  QString path = ProfileManager::activeProfile().dataDir(Names::DataName() + ".conf");
  if (path.isEmpty())
    return QVariant();
...
void SettingsComponent::load()
{
  ...
  loadConf(ProfileManager::activeProfile().dataDir(Names::DataName() + ".conf"), false);
...
bool SettingsComponent::resetAndSaveOldConfiguration()
{
  ...
  return settingsFile.rename(ProfileManager::activeProfile().dataDir(Names::DataName() + ".conf.old"));
}
...
QString SettingsComponent::getClientName()
{
  ...
  if (name.compare("TigerestTheater") == 0) {
