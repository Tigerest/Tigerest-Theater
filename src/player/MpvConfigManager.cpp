...
bool MpvConfigManager::prepare()
{
  ...
  // mpv.conf, scripts or shader directory.
  ...
  // 2.0.1 exposed the system-config switch only through a settings popup that
  // Emby 4.9.5 did not actually route to. Existing profiles therefore contain
  // the old "embedded" default even when a complete user mpv tree exists. Do
  // this one-time, persisted migration before libmpv is constructed.
  if (!settings.value(SETTINGS_SECTION_MPV, "configModeMigrated").toBool())
  {
    if (mode == "embedded" && systemConfigExists)
    {
      mode = "auto";
      settings.setValue(SETTINGS_SECTION_MPV, "configMode", mode);
      qInfo() << "Migrated MPV mode to automatic user-config detection";
    }
